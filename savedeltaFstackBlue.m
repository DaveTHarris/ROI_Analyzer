function  savedeltaFstackBlue( handles )
...
for k = minZ-minZ+1:maxZ-minZ
    ...
    if ~isempty(handles.totalROIdataSlice{k,1})
        ...
        mask(:,:,k)=max(masktemp,[],3);
    else
        mask(:,:,k)=zeros(512,512);
    end
    ...
    for i = minAxis-minAxis+1:maxAxis-minAxis
        ...
        %deltaFimage(:,:,i,k)=(Images(:,:,i,k)-Images(:,:,2,k))./Images(:,:,2,k);
         ...
         deltaFimage(:,:,i,k)=immultiply(A,mask(:,:,k));
         deltaFimage2(:,:,i,k)=immultiply(B,mask(:,:,k));
    end
end
